...
from peft_pretraining.modeling_llama_test import LlamaForCausalLM
...
def parse_args(args):
    ...
    parser.add_argument("--lr_muon", type=float, default=5e-2)
    parser.add_argument("--lr_adam_embed", type=float, default=6e-1)
    parser.add_argument("--lr_adam_scalar", type=float, default=4e-2)
    parser.add_argument("--lr_adam_head", type=float, default=8e-3)
...
def main(args):
    ...
    # show model layers
    model_layers_dict = {}
    for p_i, (p_name, p_tensor) in enumerate(model.named_parameters()):
        model_layers_dict[p_i] = {"name": p_name, "shape": p_tensor.shape}
        print(p_i, p_name, p_tensor.shape)
    with open(os.path.join(log_folder, "model_layers.json"), "w") as f:
        json.dump(model_layers_dict, f)
    print(f"Model structure saved in {log_folder}")
